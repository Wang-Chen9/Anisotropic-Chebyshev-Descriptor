...
from datasets.faust_dataloader import FAUST
...
parser.add_argument('--n_corr_points', default=6890, type=int,
...
parser.add_argument('--saving_name', '--sn', default='CSMCNN_Faust', type=str,
...
path = osp.join(osp.abspath('.'), 'datasets', 'FAUST')
...
train_dataset = FAUST(path, True, None, pre_transform)
test_dataset = FAUST(path, False, None, pre_transform)
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        if flag:
            ...
            loss=F.nll_loss(x[0:d.num_nodes, :], target) # + 1e-2*nloss
            loss.backward()
        optimizer.step()
...
mat_path = osp.join(path_output, 'tr_reg_{0:03d}.mat')
model.eval()
with torch.no_grad():
